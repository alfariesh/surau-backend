...
func TestQuranPageProfileSQLIsReadOnlyAndBounded(t *testing.T) {
	...
	assert.Contains(t, contents, "PREPARE quran_citable_hydration(text[])")
	assert.Contains(t, contents, "plan_cache_mode = 'force_custom_plan'")
	assert.Contains(t, contents, "plan_cache_mode = 'force_generic_plan'")
	assert.Contains(t, contents, "EXECUTE quran_citable_hydration(:page_ayah_keys)")
...
func TestQuranPageDiagnosticsKeepsHardSafetyGuards(t *testing.T) {
	...
	assert.Contains(t, contents, `capture_ingress_state`)
	assert.Contains(t, contents, `nginx-api-routing`)
	assert.Contains(t, contents, `api\.surau\.org`)
	assert.Contains(t, contents, `:(80|443|8080|18080|18081)`)
	...
	assert.Contains(t, contents, `shared_blk_read_time`)
	assert.Contains(t, contents, `shared_blk_write_time`)
	assert.Contains(t, contents, `stddev_exec_time`)
	assert.Contains(t, contents, `if ! capture_database_state before`)
	assert.Contains(t, contents, `if ! capture_database_state after`)
	assert.NotContains(t, contents, `       blk_read_time, blk_write_time`)
...
func TestQuranPagePerformanceWorkflowPreservesFailureEvidence(t *testing.T) {
	...
	assert.Contains(t, contents, `public_c10_requests=20`)
	assert.Contains(t, contents, `public_c10_requests=50`)
	assert.Contains(t, contents, `-concurrency 10 -query 'view=reader_minimal'`)
