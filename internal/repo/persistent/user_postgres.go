...
func (r *UserRepo) Store(ctx context.Context, user *entity.User) error {
	sql, args, err := r.Builder.
		Insert("users").
		Columns("id, username, email, role, password_hash, created_at, updated_at").
		Values(user.ID, user.Username, user.Email, user.Role, user.PasswordHash, user.CreatedAt, user.UpdatedAt).
		ToSql()
	if err != nil {
...
func (r *UserRepo) SetRoleByEmail(ctx context.Context, email, role string) (entity.User, error) {
	sqlText, args, err := r.Builder.
		...
		Suffix("RETURNING id, username, email, role, password_hash, created_at, updated_at").
		ToSql()
	if err != nil {
