...
func TestK1DevRolloutKeepsDefaultStateOutOfApplicationConfig(t *testing.T) {
	...
	assert.Contains(t, text, `-expected-citation-mode "$EFFECTIVE_BOOK_RAG_MODE"`)
	assert.Contains(t, text, "-forbid-legacy-fallback")
...
func TestK1DevRolloutKeepsLongCatalogCommandsObservable(t *testing.T) {
	...
	assert.Contains(t, text, "run_with_heartbeat k1-catalog-analyze")
	assert.Contains(t, text, "run_with_heartbeat k1-catalog-verification")
	analyze := strings.Index(text, "run_with_heartbeat k1-catalog-analyze")
	verify := strings.Index(text, "run_with_heartbeat k1-catalog-verification")

	require.NotEqual(t, -1, analyze)
	require.NotEqual(t, -1, verify)
	assert.Less(t, analyze, verify)
	assert.Contains(t, text, "ANALYZE citable_units")
	...
	assert.Contains(t, text, `printf '%s\n' "$VPS_SSH_KNOWN_HOSTS" > ~/.ssh/known_hosts`)
	assert.Contains(t, text, "-o StrictHostKeyChecking=yes")
	assert.Contains(t, text, "-o UserKnownHostsFile=~/.ssh/known_hosts")
	assert.NotContains(t, text, "ssh-keyscan")
